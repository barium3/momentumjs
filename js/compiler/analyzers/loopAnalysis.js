...
class CompilerLoopAnalyzer {
  ...
  buildMaxCode(code) {
    ...
    var scopeInfo = this._buildScopeInfo(ast);
    var variableDefs = this._collectVariableDefs(ast, scopeInfo);
    var context = {
      code: code,
  ...
  _buildScopeInfo(ast) {
    var semantics =
      typeof window !== "undefined" ? window.compilerSemantics : null;
    if (!semantics || !ast) {
      return {
        nodeScopes: typeof Map !== "undefined" ? new Map() : null,
      };
    }

    var nodeScopes = new Map();
    var bindingOptions = {
      functionBindingInfo: {
        callable: true,
        type: "function",
    ...
    semantics.collectHoistedBindings(ast, globalScope, bindingOptions);
    semantics.collectLexicalBindings(ast.body || [], globalScope, bindingOptions);
    ...
    function visitStatementList(statements, scope, functionScope) {
      if (!Array.isArray(statements)) return;
      for (var i = 0; i < statements.length; i++) {
        visitNode(statements[i], scope, functionScope);
      }
    }

    function visitNode(node, scope, functionScope) {
      ...
      switch (node.type) {
        case "Program":
          visitStatementList(node.body || [], scope, functionScope || scope);
          return;
        case "BlockStatement": {
          var blockScope = semantics.createScope(scope);
          remember(node, blockScope);
          semantics.collectLexicalBindings(node.body || [], blockScope, bindingOptions);
          visitStatementList(node.body || [], blockScope, functionScope);
          return;
        }
        case "FunctionDeclaration":
        case "FunctionExpression":
        case "ArrowFunctionExpression": {
          var fnScope = semantics.createScope(scope);
          remember(node, fnScope);

          if (node.id && node.id.type === "Identifier") {
            semantics.addBinding(fnScope, node.id.name, bindingOptions.functionBindingInfo);
          }

          if (node.type !== "ArrowFunctionExpression") {
            semantics.addBinding(fnScope, "arguments", {
              callable: false,
              type: "object",
            });
          }
          ...
          for (var p = 0; p < (node.params || []).length; p++) {
            semantics.addPatternBindings(fnScope, node.params[p], {
              callable: null,
              type: "unknown",
            });
          }
          ...
          if (node.body && node.body.type === "BlockStatement") {
            remember(node.body, fnScope);
            semantics.collectHoistedBindings(node.body, fnScope, bindingOptions);
            semantics.collectLexicalBindings(node.body.body || [], fnScope, bindingOptions);
            visitStatementList(node.body.body || [], fnScope, fnScope);
            return;
          }

          visitNode(node.body, fnScope, fnScope);
          return;
        }
        case "CatchClause": {
          var catchScope = semantics.createScope(scope);
          remember(node, catchScope);
          ...
          if (node.body && node.body.type === "BlockStatement") {
            remember(node.body, catchScope);
            semantics.collectLexicalBindings(node.body.body || [], catchScope, bindingOptions);
            visitStatementList(node.body.body || [], catchScope, functionScope);
            return;
          }

          visitNode(node.body, catchScope, functionScope);
          return;
        }
        case "ForStatement": {
          var loopScope = semantics.createLoopScope(scope, node.init, bindingOptions);
          remember(node, loopScope);
          if (node.init) visitNode(node.init, loopScope, functionScope);
          if (node.test) visitNode(node.test, loopScope, functionScope);
          if (node.update) visitNode(node.update, loopScope, functionScope);
          if (node.body) visitNode(node.body, loopScope, functionScope);
          return;
        }
        case "ForInStatement":
        case "ForOfStatement": {
          var eachScope = semantics.createLoopScope(scope, node.left, bindingOptions);
          remember(node, eachScope);
          if (node.left) visitNode(node.left, eachScope, functionScope);
          if (node.right) visitNode(node.right, eachScope, functionScope);
          if (node.body) visitNode(node.body, eachScope, functionScope);
          return;
        }
        case "SwitchStatement": {
          var switchScope = semantics.createScope(scope);
          remember(node, switchScope);
          if (node.discriminant) {
            visitNode(node.discriminant, scope, functionScope);
          }
          ...
          for (var c = 0; c < (node.cases || []).length; c++) {
            var switchCase = node.cases[c];
            if (!switchCase) continue;
            remember(switchCase, switchScope);
            if (switchCase.test) {
              visitNode(switchCase.test, switchScope, functionScope);
            }
            visitStatementList(switchCase.consequent || [], switchScope, functionScope);
          }
          return;
        }
        default:
          semantics.forEachChild(node, function (child) {
            visitNode(child, scope, functionScope);
          });
      }
    }

    visitNode(ast, globalScope, globalScope);
  ...
  _collectVariableDefs(ast, scopeInfo) {
    ...
    function addDef(name, expr, node) {
      ...
      defs[name].push({
        expr: expr,
        scope: self._getResolutionScope(node, scopeInfo),
        start: node.start,
  ...
  _getResolutionScope(node, scopeInfo) {
    return this._getNearestScope(node, scopeInfo);
  }

  _getScopeChain(node, context) {
    var chain = [];
    var current = node;
    var scopeInfo = context ? context.scopeInfo : null;
    while (current) {
      var scope = this._getNearestScope(current, scopeInfo);
      if (scope && chain.indexOf(scope) === -1) {
        chain.push(scope);
      }
      current = current.parent;
    }
  ...
  _getNearestScope(node, scopeInfo) {
    var scopes = scopeInfo && scopeInfo.nodeScopes ? scopeInfo.nodeScopes : null;
    var current = node;

    while (current) {
      if (scopes && scopes.has(current)) {
        return scopes.get(current);
      }
      current = current.parent;
    }

    return scopeInfo && scopeInfo.globalScope ? scopeInfo.globalScope : null;
  }
  ...
  _resolveIdentifierDefs(name, refNode, context) {
    ...
    var scopeChain = this._getScopeChain(refNode, context);
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = CompilerLoopAnalyzer;
}

window.CompilerLoopAnalysis = CompilerLoopAnalyzer;
