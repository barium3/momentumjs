...
- Bitmap mode uses Metal acceleration on macOS. On Windows it follows the GPU
  ...
  Momentum falls back to its CPU renderer.

Momentum does not require end users to install the CUDA toolkit. The Windows
CUDA path loads the driver API supplied by the installed NVIDIA display driver.
An After Effects "System Compatibility Report" is generated by AE before
Momentum renders; it should be handled as a host/driver issue rather than as a
Momentum backend-selection error. Keep the display driver within the range
supported by the installed AE version, and check virtual display adapters when
AE reports an unexpected GPU.
