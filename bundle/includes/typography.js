...
function getTextShapeLib() {
  return [
    ...
    "  // 添加字体信息，供 baseline shift 表达式使用",
    "  // 使用默认字体 Arial（与前端 codeExecutor 中的默认值一致）",
...
function _getTextPositionExpr(indexFind) {
  return [
    ...
    "var hasPseudoBox = (wh && wh.length > 0 && typeof wh[0] === 'number' && wh[0] > 0);",
    "",
...
function _getAnchorPointExpr(indexFind) {
  return [
    ...
    "if (rm === null || rm === undefined) { rm = 2; }",
    "var wh = (shape && shape.wh) ? shape.wh : null;",
    "var hasPseudoBox = (wh && wh.length > 0 && wh[0] > 0);",
...
function getTypographyLib(deps) {
  ...
  if (deps.text || deps.textSize || deps.textLeading) {
...
function createTextLayerFromContext(index, shapeId, mainCompName, shapeData) {
  ...
  textProp.expression = [
      ...
      "",
      "// 将一行拆成 token：",
      "// - CJK 等非 ASCII：逐字符作为独立 token（按字截断）",
      "// - 西文：按空格分词，单词整体作为一个 token，空格也保留为 token",
      ...
      "  // 高度裁剪也保持至少 1 行（与“一行一个 word 是底线”的体验一致）",
  ...
  transform.property("Anchor Point").expression = _getAnchorPointExpr(indexFind);
