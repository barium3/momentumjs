...
function getTextStateLib() {
  return [
    ...
    "// 默认使用 Arial 的 PostScript 名称，确保 AE 兼容",
    ...
    "var _textWrap = WORD;",
    "",
    "// textStyle constants (p5.js compatible)",
    "var NORMAL = 'NORMAL';",
    "var BOLD = 'BOLD';",
    "var ITALIC = 'ITALIC';",
    ...
    "}",
    "",
    "// textAlign 状态变量（数字，使用 math.js 中已声明的常量）",
    "// 默认值：水平 LEFT(1)，垂直 BASELINE(3)（与 p5.js 保持一致）",
    ...
    "  // 水平对齐：仅支持数字（CENTER=0, LEFT=1, RIGHT=2）",
    "  if (typeof h === 'number' && (h === LEFT || h === CENTER || h === RIGHT)) {",
    "    _textAlignH = h;",
    "  }",
    "  // 垂直对齐（可选）：支持 CENTER=0, TOP, BOTTOM, BASELINE",
    ...
    "}",
    "",
    "// textWidth(str) - 计算字符串在当前字体/字号下的宽度",
    "// p5.js 行为：对于多行文本，返回最宽那一行的宽度",
    "// 依赖 _ctx.fontMetrics 数据（由前端传递）",
    ...
    "  ",
    "  // 优先使用 fontMetrics 中的精确字符宽度（比例值 * 字号）",
    "  // fontKey 只用字体名，因为比例值是独立于字号的",
    "  var fontKey = _textFontFamily;",
    "  var metrics = (_ctx && _ctx.fontMetrics && _ctx.fontMetrics[fontKey]) || null;",
    "  ",
    ...
    "  ",
    "  // 回退：使用与换行逻辑一致的粗略宽度估算：",
    "  // - 空格按 0.33em 处理，避免尾部多出过宽的“不可见字符”",
    "  // - ASCII 可见字符按 0.55em 处理",
    "  // - 非 ASCII（含 CJK）按 1em 处理",
    "  var estimate = 0;",
    "  for (var j = 0; j < s.length; j++) {",
    "    var ch = s.charAt(j);",
    "    if (ch === ' ') {",
    "      estimate += _textSize * 0.33;",
    "    } else {",
    "      var code = ch.charCodeAt(0);",
    "      if (code <= 0x007f) {",
    "        estimate += _textSize * 0.55;",
    "      } else {",
    "        estimate += _textSize;",
    "      }",
    "    }",
    ...
    "}",
    "",
    "// textAscent() - 返回当前字体的上升高度",
    "// fontMetrics 中的 ascent/descent 是比例值，需要乘以当前字号",
    "function textAscent() {",
    "  var fontKey = _textFontFamily;",
    "  var metrics = (_ctx && _ctx.fontMetrics && _ctx.fontMetrics[fontKey]) || null;",
    "  if (metrics && metrics.ascent !== undefined) {",
    "    return metrics.ascent * _textSize;",
    "  }",
    "  // 回退：默认 ascent 约为 fontSize 的 0.8 倍",
    "  return _textSize * 0.8;",
    "}",
    "",
    "// textDescent() - 返回当前字体的下降高度",
    "function textDescent() {",
    "  var fontKey = _textFontFamily;",
    "  var metrics = (_ctx && _ctx.fontMetrics && _ctx.fontMetrics[fontKey]) || null;",
    "  if (metrics && metrics.descent !== undefined) {",
    "    return metrics.descent * _textSize;",
    "  }",
    "  // 回退：默认 descent 约为 fontSize 的 0.2 倍",
    "  return _textSize * 0.2;",
    "}",
  ].join("\n");
}
...
function createTextLayerFromContext(index, shapeId, mainCompName, shapeData) {
  ...
  textProp.expression = [
    ...
    "// 从 _ctx.fontMetrics 读取 ascent / descent 和 baselineOffsetRatio",
    "var fontMetrics = _ctx && _ctx.fontMetrics && _ctx.fontMetrics[fontFamily];",
    "var baselineOffsetRatio = (fontMetrics && fontMetrics.baselineOffsetRatio) || 0;",
    "var ascent = (fontMetrics && fontMetrics.ascent) || 0.8;",
    "var descent = (fontMetrics && fontMetrics.descent) || 0.2;",
    "// vertAlign: CENTER=0, TOP=1, BOTTOM=2, BASELINE=3（水平/垂直共用 CENTER=0，通过 shape.alignment.h/v 区分）",
    "var vertAlign = shape && shape.alignment && shape.alignment.v !== undefined ? shape.alignment.v : 3;",
    "// 根据 vertAlign 计算 baselineShift（与 p5.js textAlign 对齐）",
    "var baselineShift;",
    "if (vertAlign === 3) {",
    "  // BASELINE: 不偏移",
    "  baselineShift = 0;",
    "} else if (vertAlign === 2) {",
    "  // BOTTOM: 文本底部对齐 y（baseline 向下移动 descent）",
    "  baselineShift = descent * s;",
    "} else if (vertAlign === 1) {",
    "  // TOP: 文本顶部对齐 y（baseline 向上移动 ascent）",
    "  baselineShift = -ascent * s;",
    "} else if (vertAlign === 0) {",
    "  // CENTER: 垂直居中",
    "  baselineShift = (descent - ascent) / 2 * s;",
    "} else {",
    "  // 默认 BASELINE",
    "  baselineShift = 0;",
    "}",
    ...
    "}",
    "",
    'var _lines = _wrapText(_t, (typeof maxW === "number" && maxW === maxW) ? maxW : Infinity);',
    "",
    ...
    "// 支持数字常量 CENTER=0, LEFT=1, RIGHT=2",
