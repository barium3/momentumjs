...
function getBackgroundLib() {
  return [
    ...
    "  var fill1 = [c[0], c[1]];",
    "  var fill2 = [c[2], c[3] !== undefined ? c[3] : 1];",
    "  var mk = [m, 1005];",
    "  _out = _out.concat([fill1, fill2, mk]);",
    "}",
  ].join("\n");
}
 ...
 */
function createBackgroundFromContext(index, shapeIndex) {
  ...
  fill.property("Color").expression = [
    ...
    "var f1 = pts[markerIdx - 2], f2 = pts[markerIdx - 1];",
    "if (!f1 || f1[0] < 0) [1, 1, 1, 1];",
    "[f1[0], f1[1], f2[0], 1]",
  ].join("\n");
