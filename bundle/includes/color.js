...
function getFillStrokeLib() {
  return [
    ...
    "  if (Array.isArray(arguments[0])) { _fillColor = arguments[0]; }",
    "  else { _fillColor = color.apply(null, arguments); }",
    "  _noFill = false;",
    "}",
    "function stroke() {",
    "  if (arguments.length === 0) return;",
    "  if (Array.isArray(arguments[0])) { _strokeColor = arguments[0]; }",
...
function getColorLib(deps) {
  ...
  if (deps.fill) {
    ...
    lib.push("function fill() { if (arguments.length === 0) return; if (Array.isArray(arguments[0])) { _fillColor = arguments[0]; } else { _fillColor = color.apply(null, arguments); } _noFill = false; }");
  }
  if (deps.stroke) {
    if (!deps.fill) lib.push(getColorFuncLib());  // 只在还没定义 color 时添加
    lib.push("function stroke() { if (arguments.length === 0) return; if (Array.isArray(arguments[0])) { _strokeColor = arguments[0]; } else { _strokeColor = color.apply(null, arguments); } _noStroke = false; }");
  }
...
function getColorFunctionNames() {
  ...
  // 备用：硬编码列表
  return ["fill", "stroke", "noFill", "noStroke", "strokeWeight", "color", "lerpColor"];
}
