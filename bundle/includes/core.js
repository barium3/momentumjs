...
function getEngineStateLib(
  ...
  return [
    "function _readTextDocumentValue(raw) {",
    "  return (raw && raw.text !== undefined) ? raw.text : (raw && raw.toString ? raw.toString() : raw);",
    "}",
    "function _createFreshEngineContext() {",
    "  return {",
    "    fps: fps,",
    "    frame: currentFrame,",
    "    time: currentTime,",
    ...
    "  try {",
    "    var previousRaw = thisProperty.valueAtTime(currentTime - thisComp.frameDuration);",
    "    var previousJson = _readTextDocumentValue(previousRaw);",
    ...
    "  if (!next.drawShapes) next.drawShapes = [];",
    "  if (!next.setupBackgrounds) next.setupBackgrounds = next.backgrounds ? next.backgrounds.slice(0) : [];",
    "  if (!next.drawBackgrounds) next.drawBackgrounds = [];",
    "  if (!next.globals || typeof next.globals !== 'object') next.globals = {};",
    ...
    "    _shapes = _ctx.setupShapes;",
    "    _backgrounds = _ctx.setupBackgrounds;",
    "    return;",
    "  }",
    "  if (phase === 'draw') {",
    "    _ctx.drawShapes = [];",
    "    _ctx.drawBackgrounds = [];",
    "    _shapes = _ctx.drawShapes;",
    "    _backgrounds = _ctx.drawBackgrounds;",
    "    return;",
    "  }",
    "  _refreshMergedRenderState();",
    "}",
    "function _refreshMergedRenderState() {",
    "  var setupShapes = _ctx.setupShapes || [];",
    "  var drawShapes = _ctx.drawShapes || [];",
    "  var setupBackgrounds = _ctx.setupBackgrounds || [];",
    "  var drawBackgrounds = _ctx.drawBackgrounds || [];",
    "  _ctx.shapes = setupShapes.concat(drawShapes);",
    "  _ctx.backgrounds = setupBackgrounds.concat(drawBackgrounds);",
    "  _shapes = _ctx.shapes;",
    "  _backgrounds = _ctx.backgrounds;",
    "}",
    "var _ctx = _normalizeEngineContext(_readPreviousEngineContext());",
