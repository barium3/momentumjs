...
function continueTest() {
  ...
  img.onload = function () {
    ...
    if (typeof window.classifier === "undefined") {
      console.error("classifier 仍未定义。请检查 momentum.js 是否正确加载");
      return;
    }
...
function gotResult(results, error) {
  ...
  // 检查 momentum.js 库是否已加载
  if (!checkMomentumLibrary()) {
    return;
  }
...
// 检查 m 对象是否已定义
function checkMomentumLibrary() {
  if (typeof m === "undefined") {
    console.error("momentum.js 库未加载或初始化。请确保正确引入了 momentum.js");
    return false;
  }
  return true;
}
